...
The reasoning behind this claim is that we have used Python, a scripting language, to solve the problem which is generally slower than a compiled language such as FORTRAN or C.

In order to gauge any benefits that a compiled language might bring, the Python script was timed over several runs of numerically integrating and then summing the separate differential cross sections. The time was averaged to give a rough runtime estimate. After 50 runs of the script, the average runtime was AVG RUNTIME HERE.

A C script was then written which performed the same numerical integrations as the Python script, and again was ran 50 times to produce an average runtime. It was found that the C script took an average of 
